...
function decode(data, crc) {
  var newBytes = new Array(), escaped = false, val;
  for (var i=0,len=data.length; i<len; ++i) {
    if (data[i] === 61) {
      ...
      // CRC update
      crc.val = (crc.val >>> 8) ^ (crc_tab[(crc.val ^ val) & 0xff]);
    }
  }
      ...
      if (eolPos - startPos > 0) {
        ...
        if (state === 'pre' && line.startsWith(yBegin)) {
          ...
          if (result = line.toString().match(/^=ybegin (.+?) name=(.+)$/)) {
            ...
            if (typeof meta.begin.part !== 'undefined')
              state = 'part';
            else
              state = 'data';
          ...
          if (line.startsWith(yEnd)) {
            ...
            if (result) {
              ...
              var ycrc = undefined;
              if (typeof meta.end.pcrc32 !== 'undefined')
                ycrc = parseInt(meta.end.pcrc32, 16);
              else if (typeof meta.end.crc32 !== 'undefined')
                ycrc = parseInt(meta.end.crc32, 16);
              crc.val = crc.val ^ (-1);
              if (crc.val < 0)
                crc.val += 0xFFFFFFFF + 1;
              if (typeof ycrc !== 'undefined')
                meta.end.crcMatches = (crc.val === ycrc);
              else
                meta.end.crcMatches = undefined;
              state = 'pre';
              crc.val = -1;
              isHeader = true;
          ...
          if (!isHeader)
            outstream.emit('data', decode(line, crc));
        }
        line = undefined;
      }
      startPos = eolPos + 2;
