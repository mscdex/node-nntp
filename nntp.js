              ...
              if (respsHaveArgs.indexOf(code) > -1) {
                if (isML){
                  self._callCb(self._MLEmitter, text);
                	console.log("Calling Callback - isML: " + isML);
                }
                else{
...
NNTP.prototype.post = function(msg, cb) {
  ...
  if (!this._state || !msg || !Object.keys(msg).length){
    console.log("Returning False at the start of the post function");
    return false;
  }

  var self = this, composing = true;
  console.log("Message Posting: composing = True");
  return this.send('POST', function(e) {
    if (e || !composing){
    	console.log("*** About to return the callback value");
      	return cb(e);
    }
    composing = false;
    console.log("Composing = false");
    var CRLF = '\r\n',
  ...
  console.log("Post of message complete?");
...
NNTP.prototype.send = function(cmd, params, cb) {
	console.log("Entering send function - Cmd: " + cmd);
  if (!this._socket || !this._socket.writable){
    	console.log("Returning False: Socket doesn't exist or isn't writable");
    	return false;
    }
  ...
  console.log("This.queue: " + this._queue);
  ...
  console.log("About to return true from send function");
  return true;
};
